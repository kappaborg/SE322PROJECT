...
class HomePage extends BasePage {
	constructor(page) {
		...
		this.searchFieldInput = "#txtSearch";
	}
	...
	async isLoggedIn() {
		...
		return (
			(await this.isVisible(this.userMenu)) ||
			(await this.isVisible(this.welcomeMessage)) ||
			!this.getCurrentUrl().includes("login.aspx")
		);
	}

	async searchField(searchText) {
		await this.fill(this.searchFieldInput, searchText);
	}
	...
	async testHomePageSearch(searchText) {
		const input = this.page.locator(this.searchFieldInput);
